...
# BOZUK Module Preservation and Robustness Analysis

Şu an bozuk burası


```{r}
## The goal is to test if the modules discovered are robust and reproducible.
## We will split the data into two random halves: a 'discovery' set and a 'test' set.
## We then check if the modules found in the discovery set are preserved in the test set.
#
#conflicts_prefer(base::setdiff)
#
##
## --- NEW CHUNK: MODULE PRESERVATION AND ROBUSTNESS ANALYSIS (REVISED) ---
##
#
## For reproducibility of the random split
#set.seed(123)
#
## --- 1. Create two random subsets of the data ---
#n_samples <- nrow(datExpr)
#discovery_indices <- sample(1:n_samples, size = round(n_samples / 2))
#test_indices <- setdiff(1:n_samples, discovery_indices)
#
#discovery_data <- datExpr[discovery_indices, ]
#test_data <- datExpr[test_indices, ]
#
## --- 2. IMPORTANT: Check for and remove zero-variance genes in each subset ---
## Splitting the data can sometimes result in genes with no variance in one of the subsets.
## WGCNA requires genes to have some variance to be analyzed.
#discovery_data <- discovery_data[, apply(discovery_data, 2, var) > 0]
#test_data <- test_data[, apply(test_data, 2, var) > 0]
#
## Ensure the gene lists are identical between the two sets after cleaning
#common_genes_split <- intersect(colnames(discovery_data), colnames(test_data))
#discovery_data <- discovery_data[, common_genes_split]
#test_data <- test_data[, common_genes_split]
#
#cat("Dimensions of discovery data:", dim(discovery_data), "\n")
#cat("Dimensions of test data:", dim(test_data), "\n")
#
#
## --- 3. Run WGCNA on the discovery set to get reference modules ---
#net_discovery <- blockwiseModules(
#  discovery_data,
#  power = chosen_power,
#  TOMType = "signed",
#  minModuleSize = 30,  # Increased minModuleSize slightly for robustness in smaller dataset
#  mergeCutHeight = 0.4,
#  numericLabels = TRUE,
#  verbose = 2
#)
#
## --- 4. CRITICAL CHECK: Verify that modules were actually found ---
#discovery_colors <- net_discovery$colors
#cat("\n--- Modules found in the discovery set: ---\n")
#print(table(discovery_colors))
#
## Check if there are any modules other than the grey module (0)
#if (length(unique(discovery_colors)) <= 1) {
#  stop("Error: No modules were detected in the discovery dataset (all genes are grey).
#       Cannot proceed with module preservation analysis.
#       Consider using a larger dataset or adjusting WGCNA parameters.")
#}
#
#
## --- 5. Run the Module Preservation Analysis ---
#setLabels <- c("Discovery", "Test")
#multiExpr <- list(Discovery = list(data = discovery_data), Test = list(data = test_data))
#multiColor <- list(Discovery = discovery_colors)
#
#mp <- modulePreservation(
#  multiData = multiExpr,
#  multiColor = multiColor,
#  referenceNetworks = 1,
#  nPermutations = 100, # Using 200 is more robust
#  randomSeed = 1,
#  quickCor = 0,
#  verbose = 3
#)
...
#stats <- mp$preservation$Z$ref.Discovery$inColumns.Test
#stats_summary <- stats[, c("module", "Zsummary.pres")]
#print("\n--- Module Preservation Z-summary Scores ---")
#print(stats_summary)
#
## Plot the Z-summary scores, excluding the unassigned 'gold' (grey) module
#plot_stats <- stats[stats$module != "gold", ] # 'gold' is the label for the grey module in plots
#
#plot(plot_stats$moduleSize, plot_stats$Zsummary.pres,
#     col = plot_stats$module,
#     pch = 19,
#     cex = 1.5,
#     xlab = "Module Size",
#     ylab = "Z-summary Preservation Score",
#     main = "Module Preservation in Test Set"
#)
#
## Add a horizontal line for the Z=10 and Z=2 thresholds
#abline(h = 10, col = "blue", lty = 2)
#abline(h = 2, col = "red", lty = 2)
#legend("topright", legend = c("Strong Preservation (Z > 10)", "Weak Preservation (Z > 2)"),
...
if (!dir.exists("GSE169471_RAW")) {
  dir.create("GSE169471_RAW")
}
getGEOSuppFiles("GSE169471", makeDirectory = TRUE, baseDir = ".")

# Unzip the main tar file
untar("GSE169471/GSE169471_RAW.tar", exdir = "GSE169471_RAW")
...
data_gsm1 <- Read10X(data.dir = "GSE169471_RAW/GSM5206779_PAH_P1_filtered_feature_bc_matrix/")
data_gsm2 <- Read10X(data.dir = "GSE169471_RAW/GSM5206780_PAH_P2_filtered_feature_bc_matrix/")
data_gsm3 <- Read10X(data.dir = "GSE169471_RAW/GSM5206781_PAH_P3_filtered_feature_bc_matrix/")
